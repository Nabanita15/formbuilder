...
const Comprehense = () => {
  ...
  const handleSubmit = async () => {
    try {
      await axios.post("http://localhost:5000/api/comprehension", form);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/api/comprehension"
      );
 ...
 const handleDelete = async (id) => {
   try {
     await axios.delete(`http://localhost:5000/api/comprehension/${id}`);
     console.log("delete");
     fetchData(); 
   } catch (error) {
